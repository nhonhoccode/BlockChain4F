...
const chairmanService = {
  ...
  getImportantDocuments: async (filters = {}) => {
    try {
      ...
      // Xây dựng URL với query params
      const url = queryParams.toString() 
        ? `${API_ENDPOINTS.CHAIRMAN.IMPORTANT_DOCUMENTS}?${queryParams.toString()}`
        : API_ENDPOINTS.CHAIRMAN.IMPORTANT_DOCUMENTS;
      
      // Gọi API thực tế để lấy danh sách giấy tờ quan trọng
      try {
        const response = await get(url);
        console.log('Important documents API response:', response);
        return response;
      } catch (error) {
        // API không tồn tại hoặc có lỗi server, sử dụng mock data
        console.warn('API endpoint not found or server error:', error.message);
  ...
  approveDocument: async (documentId, data = {}) => {
    try {
      ...
      // Gọi API để phê duyệt tài liệu
      const url = API_ENDPOINTS.CHAIRMAN.APPROVE_DOCUMENT(documentId);
      try {
  ...
  rejectDocument: async (documentId, data = {}) => {
    try {
      ...
      // Gọi API để từ chối tài liệu
      const url = API_ENDPOINTS.CHAIRMAN.REJECT_DOCUMENT(documentId);
      try {
        const response = await post(url, data);
        return response;
      } catch (error) {
