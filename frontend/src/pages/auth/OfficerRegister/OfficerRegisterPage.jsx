...
// Mock API đăng ký cán bộ
const mockRegisterOfficer = (data) => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      resolve({
        success: true,
        requestId: 'OFF-REQ-' + Math.floor(10000 + Math.random() * 90000),
...
const OfficerRegisterPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await mockRegisterOfficer(formData);
      setSuccess(true);
      setRequestId(response.requestId);
    } catch (err) {
      console.error('Registration error:', err);
      setError(err.message || 'Có lỗi xảy ra khi đăng ký. Vui lòng thử lại sau.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <Container maxWidth="sm" sx={{ py: 8 }}>
        <Paper elevation={3} sx={{ p: 4, textAlign: 'center' }}>
          ...
          <Typography variant="body1" paragraph>
            Cảm ơn bạn đã đăng ký làm cán bộ xã. Đơn đăng ký của bạn đã được gửi thành công.
          </Typography>
          <Typography variant="body1" paragraph>
            Mã đơn đăng ký của bạn là:
          </Typography>
          <Typography variant="h5" sx={{ fontWeight: 'bold', my: 3, color: 'primary.main' }}>
            {requestId}
          </Typography>
          <Typography variant="body2" paragraph>
            Chúng tôi sẽ xem xét đơn đăng ký của bạn và liên hệ lại trong thời gian sớm nhất.
            Vui lòng lưu lại mã đơn đăng ký để theo dõi tình trạng xử lý.
          </Typography>
