...
const OfficerApproval = () => {
  ...
  useEffect(() => {
    if (searchTerm) {
      const filtered = approvals.filter(
        approval => {
          const name = approval.user_name || approval.name || ''; 
          const position = approval.position || approval.requested_role || '';
  ...
  const fetchApprovals = async () => {
    ...
    try {
      // Sử dụng API thật thay vì dữ liệu giả
      const response = await chairmanService.getOfficerApprovals({ status: 'pending' });
      
      if (response && Array.isArray(response)) {
        setApprovals(response);
        setFilteredApprovals(response);
      } else {
        // Fallback vào mock data nếu API chưa hoạt động
        console.warn('API không trả về dữ liệu đúng định dạng, sử dụng dữ liệu mẫu');
      ...
      // Fallback vào mock data nếu API lỗi
      setApprovals(mockApprovals);
      setFilteredApprovals(mockApprovals);
    } finally {
      setLoading(false);
  ...
  const handleApprovalAction = async () => {
    try {
      if (dialogAction === 'approve') {
        // Sử dụng API thật để phê duyệt
        const response = await chairmanService.approveOfficer(selectedApproval.id, { 
          notes: dialogReason || 'Phê duyệt bởi chủ tịch xã' 
        });
        
        if (response && response.success) {
          // Cập nhật UI sau khi phê duyệt thành công
          ...
          throw new Error(response?.message || 'Không thể phê duyệt cán bộ');
        }
      } else {
        // Sử dụng API thật để từ chối
        if (!dialogReason) {
        ...
        const response = await chairmanService.rejectOfficer(selectedApproval.id, { 
          reason: dialogReason,
          notes: dialogReason
        });
        ...
        if (response && response.success) {
          ...
          throw new Error(response?.message || 'Không thể từ chối cán bộ');
        }
      }
      ...
      setSnackbarMessage(`Lỗi: ${err.message || `Không thể ${dialogAction === 'approve' ? 'phê duyệt' : 'từ chối'} cán bộ`}`);
  ...
  const getStatusChip = (status) => {
    switch (status) {
      ...
      case 'additional_info_required':
        return <Chip label="Yêu cầu bổ sung" color="secondary" size="small" />;
      default:
                ...
                <CardHeader
                  avatar={
                    <Avatar sx={{ bgcolor: theme.palette.primary.main }}>
                      {(approval.user_name || approval.name || '?').charAt(0)}
                    </Avatar>
                  }
                  title={approval.user_name || approval.name || 'Không có tên'}
                  subheader={approval.position || approval.requested_role || 'Không có chức vụ'}
                  action={
                    <Box>
                ...
                <CardContent>
                  <Box sx={{ mb: 2 }}>
                    <Typography variant="body2" color="text.secondary" gutterBottom>
                      <strong>Email:</strong> {approval.user?.email || approval.email || 'Không có email'}
                    </Typography>
                    <Typography variant="body2" color="text.secondary" gutterBottom>
                      <strong>Điện thoại:</strong> {approval.phone || 'Không có số điện thoại'}
                    </Typography>
                    <Typography variant="body2" color="text.secondary" gutterBottom>
                      <strong>Ngày đăng ký:</strong> {formatDate(approval.submitted_date || approval.requestDate || new Date())}
                    </Typography>
                  </Box>
        ...
        <DialogContent>
          <DialogContentText sx={{ mb: 2 }}>
            {dialogAction === 'approve'
              ? `Bạn có chắc chắn muốn phê duyệt đơn đăng ký của ${selectedApproval?.user_name || selectedApproval?.name || ''}?`
              : `Bạn có chắc chắn muốn từ chối đơn đăng ký của ${selectedApproval?.user_name || selectedApproval?.name || ''}?`
            }
          </DialogContentText>
