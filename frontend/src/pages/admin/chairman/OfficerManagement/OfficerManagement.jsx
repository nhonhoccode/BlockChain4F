...
const OfficerManagement = ({ onEdit, onDelete, onView, onRefresh }) => {
  ...
  const fetchOfficers = async () => {
    ...
    try {
      ...
      // const response = await chairmanService.getOfficers();
  ...
  const handleDeleteOfficer = async () => {
    try {
      // In production, use actual API call
      // await chairmanService.deleteOfficer(selectedOfficer.id);
      
      // Update local state for now
      const updatedOfficers = officers.filter(officer => officer.id !== selectedOfficer.id);
      setOfficers(updatedOfficers);
      
      setSnackbarMessage(`Đã xóa cán bộ ${selectedOfficer.name} thành công`);
      ...
      setSnackbarMessage('Lỗi: Không thể xóa cán bộ');
  ...
  const handleBlockOfficer = async () => {
    try {
      const newStatus = selectedOfficer.status === 'active' ? 'inactive' : 'active';
      
      // In production, use actual API call
      // await chairmanService.updateOfficerStatus(selectedOfficer.id, { status: newStatus });
      
      // Update local state for now
      const updatedOfficers = officers.map(officer => 
        officer.id === selectedOfficer.id ? { ...officer, status: newStatus } : officer
      );
      setOfficers(updatedOfficers);
      
      setSnackbarMessage(
        newStatus === 'active' 
          ? `Đã kích hoạt tài khoản của ${selectedOfficer.name}` 
          : `Đã vô hiệu hóa tài khoản của ${selectedOfficer.name}`
      );
      setSnackbarSeverity('success');
      setSnackbarOpen(true);
      ...
      setSnackbarMessage('Lỗi: Không thể cập nhật trạng thái cán bộ');
      setSnackbarSeverity('error');
      setSnackbarOpen(true);
